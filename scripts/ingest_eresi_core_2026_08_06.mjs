...
async function upsertProgram(lenderId, name, config, label, effectiveDate, sourceUrl, status, createdBy) {
  ...
  const { data: gv, error: gvFindError } = await admin.from("guideline_versions").select("id,verification_status").eq("program_id", id).eq("label", label).limit(1);
  if (gvFindError) throw gvFindError;
  if (gv?.[0]) {
    const { error } = await admin.from("guideline_versions").update({
      effective_date: effectiveDate, last_verified_date: REVIEWED, verification_status: status,
...
function programs(lenderId) {
  ...
  return [
    ...
    ["Core Jumbo Express AUS", common(lenderId, { incomeDocTypes: ["full_doc"], loanPurposes: PURPOSES, occupancies: ["primary", "second_home", "investment"], propertyTypes: ["single_family", "pud", "townhome", "condo", "non_warrantable_condo", "2_4_unit", "rural"], citizenshipEligible: CORE_CITIZENSHIP, vestingEligible: ["individual", "joint_tenants", "trust"], minLoanAmount: 400000, maxLoanAmount: 3000000, minFico: 660, maxDti: 49.99, baseMaxLtv: 80, minReservesMonths: 6, interestOnlyAvailable: false, prepaymentPenaltyOptions: ["none"], propertyTypeLtvCaps: { condo: 80, non_warrantable_condo: 70, "2_4_unit": 80 }, sourceCitation: `${DOC.jumboMatrix} p.1; ${DOC.jumboGuide} Version 1.0`, sourceDocuments: [DOC.jumboMatrix, DOC.jumboGuide], currentVersionPending: true, excludedFromVerifiedMatching: true, notes: "QUARANTINED pending written confirmation that the June 25, 2025 Version 1.0 guide/matrix remain current as of August 2026. DU only; manual underwriting prohibited. Do not quote or activate as verified until lender confirmation is received." }), "eRESI Jumbo Express AUS Version 1.0 effective 2025-06-25 — pending current-version confirmation", "2025-06-25", DOC.jumboGuide, "imported_pending_review"],
  ];
}
...
async function main() {
  if (DRY_RUN) {
    ...
    assert.equal(payload.filter((p) => p.status === "human_verified").length, 12);
    assert.equal(payload.filter((p) => p.status === "imported_pending_review").length, 1);
  ...
  assert.equal(visibleVerified.length, 12);
  assert.equal(latestByProgram.get(jumbo.id)?.verification_status, "imported_pending_review");
  assert.equal(jumbo.config.currentVersionPending, true);
  console.log(`Production validation passed: eRESI Capital active with ${visibleVerified.length} verified programs; Jumbo Express AUS quarantined pending current-version confirmation.`);
}
