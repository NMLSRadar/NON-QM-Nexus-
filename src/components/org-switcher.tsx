import { listUserOrganizations, tryGetCurrentOrganizationId } from "@/lib/session";
 ...
 * selection server-side, hands both to the tiny client select. Uses the
 * try- variant (no redirect) — layout-component rule. */
export async function OrgSwitcher() {
  const organizations = await listUserOrganizations();
  if (organizations.length < 2) return null;

  const currentOrgId = await tryGetCurrentOrganizationId();
