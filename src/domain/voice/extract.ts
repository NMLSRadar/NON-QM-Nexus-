...
const CITIZENSHIP_RE = new RegExp(
  [
    ...
    // precedent as bare "ead"/"c09"/"h1b"), the STT "I-tend"/"eye-tend"/"ay
    // ten(t)" renderings (added 2026-08-25 — operators reported ITIN spoken
    // as "i tend" not registering; unconditional, same precedent), "individual
    ...
    String.raw`(?<itin>\bi[\s.-]*t[\s.-]*i[\s.-]*n\b|\beye[\s-]*tin\b|\bay[\s-]*(?:ten|10|tin|tan|teen)\b|\bey[\s-]*(?:ten|10|tin|tan)\b|\beitan\b|\baitan\b|\bayatin\b|\beyatin\b|\bayetin\b|\beyetin\b|\bi[\s-]*tend\b|\beye[\s-]*tend\b|\baye[\s-]*tend\b|\bay[\s-]*tend\b|\bey[\s-]*tend\b|\bitin\s+(?:borrower|loan|instead)\b|\btax\s?id\s+borrower\b|\bindividual taxpayer id(?:entification)?(?:\s+number)?\b|\btaxpayer identification number\b|\btax identification number\b|\bno\s+social security number\b|\b(?:does\s?n['o]?t|do\s+not|doesn['o]?t)\s+have\s+a\s+social security number\b|\bwithout\s+a\s+social security number\b)`,
...
export function extractFromTranscript(rawTranscript: string): VoiceExtraction {
  ...
  const pnlPattern = /p\s*&\s*l(?:\s+only|\s+loan|\s+program|\s+income)?|p and l(?:\s+only|\s+loan|\s+program|\s+income)?|\bpnl\b(?:\s+only|\s+loan|\s+program|\s+income)?|profit(?:\s+and)?\s+loss(?:\s+only|\s+loan|\s+program|\s+income)?|qualif(?:y|ying)\s+(?:using|with)\s+(?:a\s+)?p\s*(?:&|and)\s*l|cpa[\s-]prepared statement|accountant[\s-]prepared statement|\bno bank statements?\b/;
  const income1099Pattern = /\b1099s?\b|independent contractor(?: income)?|contractor income only|\bfreelancers?\b|\bgig workers?\b|\bgig income\b|paid (?:entirely )?(?:on|via|with) (?:a )?1099\b/;
  const assetDepletionPattern = /asset (?:depletion|utilization|based|qualifier)|asset[\s-]based income|qualify(?:ing)? off (?:their |his |her )?assets\b|using (?:their |his |her )?assets to qualify\b|no income at all\s*,?\s*just using (?:their |his |her )?(?:investment )?accounts\b|qualifying off (?:their |his |her )?(?:investment )?(?:accounts|portfolio)\b|substantial liquid assets\b|substantial (?:savings|investment portfolio)\b/;
  ...
  if (isGenuinelyAmbiguous) {
    ...
    // P&L is an exact product classification. A mention of one or two months
    // of supporting bank statements validates the P&L; it never converts the
    // request into a traditional 12/24-month Bank Statement calculation.
    x.incomeDocType = cap(IncomeDocType.ProfitAndLoss, "P&L Only");
  ...
  // Deterministic 5–8 unit product lock. This runs after both occupancy and
  // income-document classification so even contradictory language in the
  // same scenario cannot overwrite the catalog's exclusive 5–8 unit route.
  // The lock disappears automatically when a later/full transcript changes
  // the unit count/property classification outside the 5–8 range.
  if (x.propertyType?.value === PropertyType.FiveToEightUnit) {
    const unitLabel = x.units ? `${x.units}-unit property` : "5–8 unit property";
    x.occupancy = cap(Occupancy.Investment, `automatically derived from ${unitLabel}`, true);
    x.incomeDocType = cap(IncomeDocType.Dscr, `automatically derived from ${unitLabel}`, true);
  }
