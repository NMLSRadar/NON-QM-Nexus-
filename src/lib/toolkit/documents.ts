...
const assetSchema = z.object({ checking: z.number().nonnegative(), brokerage: z.number().nonnegative(), stocks: z.number().nonnegative(), cryptocurrency: z.number().nonnegative().default(0), retirement: z.number().nonnegative(), vested: z.number().min(0).max(100), down: z.number().nonnegative(), costs: z.number().nonnegative(), reserves: z.number().nonnegative(), divisor: z.number().positive() }).strict();
...
export async function createCalculatorReportPdf(calculator: ToolkitCalculatorId, rawInputs: Record<string, unknown>, borrowerReference?: string): Promise<Uint8Array> {
  ...
  if (calculator === "dscr") {
    ...
    const r = calcAssetDepletion({ checkingSavings: i.checking, brokerage: i.brokerage, stocksBonds: i.stocks, cryptocurrency: i.cryptocurrency, retirement: i.retirement, retirementVestedPercent: i.vested, requiredDownPayment: i.down, closingCosts: i.costs, requiredReserves: i.reserves, assetDivisorMonths: i.divisor }, { deductDownPayment: true, deductClosingCosts: true, deductReserves: true });
    report = { title: "Asset Depletion", subtitle: "Eligible assets after funds-to-close deductions", borrowerReference, headline: money(Number(r.value ?? 0)), rows: [["Cryptocurrency amount", money(i.cryptocurrency)], ["Eligible cryptocurrency (60%)", money(Number(r.inputs?.cryptocurrencyEligible ?? 0))], ["Eligible assets", money(Number(r.inputs?.eligibleAssets ?? 0))], ["Down payment", money(i.down)], ["Closing costs", money(i.costs)], ["Reserves", money(i.reserves)], ["Net depletable assets", money(Number(r.inputs?.netEligible ?? 0))], ["Divisor", `${i.divisor} months`]], math: [r.formula, `${money(Number(r.inputs?.netEligible ?? 0))} ÷ ${i.divisor} = ${money(Number(r.value ?? 0))}`], notes: r.notes ?? [] };
