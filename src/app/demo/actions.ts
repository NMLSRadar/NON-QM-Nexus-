...
export async function submitDemoRequest(
  ...
  try {
    ...
    const html = `
      ...
      The visitor was sent to the shared booking calendar to pick a time.</p>
    `;
  ...
  // shared scheduler (reached via a normal user tap, which avoids the
  // forced redirect chain that mobile Safari rejects).
  return { success: true, bookingUrl: DEMO_BOOKING_URL };
}
