...
export default async function AdminSpecialistsPage() {
  ...
  const programs = (data ?? [])
    .map((p) => p as unknown as ProgramRow)
    .filter((p) => {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-lg font-semibold text-slate-900">ITIN / Foreign National specialist flags</h2>
        <p className="text-sm text-slate-500">
          Editorial signal only — see the field&apos;s doc comment in <code>src/domain/types/program.ts</code>. Toggling this
          adds a small, disclosed 5-point factor to the real match score for that citizenship class and shows a
          &ldquo;Specialist&rdquo; badge on the lender card in Best Lender Matches. It NEVER overrides real eligibility — a
          program that fails a hard citizenship/LTV/etc. check is still excluded regardless of this flag, and it never
          silently reorders results on its own; a genuinely stronger real match from another lender still outranks it.
        </p>
      </div>
