...
describe("NON-QM Nexus branded toolkit documents", () => {
  ...
  it("creates an enhanced protected P&L workbook with editable expense rows and live formulas", async () => {
    const bytes = await createTemplateWorkbook("pnl");
    const workbook = new ExcelJS.Workbook();
    await workbook.xlsx.load(bytes as never);
    const sheet = workbook.worksheets[0]!;
    expect(sheet.getCell("B8").value).toBe("Business name");
    expect(sheet.getCell("B14").value).toBe("Cost of Goods Sold");
    expect(sheet.getCell("B41").value).toBe("Custom expense 4");
    expect(sheet.getCell("C45").value).toMatchObject({ formula: "SUM(C14:C41)" });
    expect(sheet.getCell("C46").value).toMatchObject({ formula: "IF(C44=0,0,C45/C44)" });
    expect(sheet.getCell("C47").value).toMatchObject({ formula: "C44-C45" });
    expect(sheet.getCell("C48").value).toMatchObject({ formula: "IF(C10=0,0,C47/C10)" });
    expect(sheet.getCell("C14").protection.locked).toBe(false);
    expect(sheet.getCell("C45").protection?.locked).not.toBe(false);
  ...
  it("creates a server-recomputed PDF for every calculator and the Reverse Solver", async () => {
    const samples: Record<ToolkitCalculatorId, Record<string, unknown>> = {
      ...
      "asset-depletion": { checking: 250000, stocks: 200000, bonds: 0, bondsInvestmentGrade: true, mutualFunds: 500000, cryptocurrency: 0, retirement: 350000, down: 200000, costs: 30000, reserves: 50000, divisor: 120 },
